<?php
    class Sudoku
    {
        ...
        public function processAction(string $action): void
        {
            if ($action === 'validar') {
                foreach ($this->grid as $linha) {
                    if (in_array('', $linha)) {$this->aviso = "Preencha todos os campos antes de validar!";return;}
                }
        ...
        private function gerarAleatorio(int $quantidade): void
        {
            $preenchidos = 0;
            while ($preenchidos < $quantidade) {
                ...
                $j   = rand(0, 8);
                $num = rand(1, 9);
                if ($this->grid[$i][$j] === '') {$this->grid[$i][$j] = $num;
                    $preenchidos++;}
            }
        }
    }
...
<h1 class="text-3xl sm:text-4xl md:text-5xl font-bold mb-6 text-blue-400 text-center">Validador de Sudoku</h1>

<?php if ($sudoku->getAviso()): ?>
<div class="mb-4 p-3 rounded bg-blue-200 text-gray-900 text-center"><?php echo $sudoku->getAviso(); ?></div>
<?php endif; ?>

<?php if ($sudoku->getResultado()): ?>
<div class="mb-4 p-3 rounded<?php echo strpos($sudoku->getResultado(), 'válido') !== false ? ' bg-gray-700 text-blue-200' : ' bg-gray-800 text-red-400'; ?> text-center">
                ...
                <?php for ($j = 0; $j < 9; $j++):
	                ...
	                <td class="border border-gray-600">
	                    <input type="text" maxlength="1" name="cell[<?php echo $i ?>][<?php echo $j ?>]"
	                           ...
	                           class="w-8 sm:w-10 md:w-12 h-8 sm:h-10 md:h-12 text-center font-bold	                                                                                                <?php echo $blockColor ?> text-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base md:text-lg"
